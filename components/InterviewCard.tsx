...
const InterviewCard = ( {
    ...
    const feedack = null as Feedback | null;
    const normalizedType = /mix/gi.test(type) ? "Mixed" : type;
    const formattedDate = dayjs(feedack ?.createdAt || createdAt || Date.now()).format("MMM D, YYYY")
  return (
        ...
        <div>
            ...
            <div className="flex flex-row gap-5 mt-3">
                ...
                <div className="flex flex-row gap-2 items-center">
                    ...
                    <p>{feedack ?.totalScore || '---'}/100</p>
                </div>
            </div>
            <p className='line-clamp-2 mt-5' >
                {feedack?.finalAssessment || "You have'nt Taken Interview yet. Take it now to improve your skills."}
            </p>
        </div>
        <div className='flex flex-row justify-between ' >
            ...
            <Button className='btn-primary' >
                <Link href={feedack
                    ...
                    {feedack ? 'Check Feedback' : 'View Interview'}
                </Link>
            </Button>
        </div>
      </div>
